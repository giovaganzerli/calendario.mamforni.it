<template>
    <div v-if="!disabled || (disabled && images.length)" style="outline: none;">
        <div class="image-container position-relative text-center" v-if="!images.length">
            ...
            <div v-else class="image-center position-absolute display-flex flex-column justify-content-center align-items-center" @dragover.prevent="onDragover">
                ...
                <div class="image-input position-absolute full-width full-height">
                    <label :for="imageUploadId" class="full-width full-height cursor-pointer">
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="image-container position-relative text-center image-list" v-else>
            ...
            <div class="image-bottom display-flex position-absolute full-width align-items-center justify-content-between" :class="!showPrimary && 'justify-content-end'" v-if="!disabled">
                    ...
                    <div class="display-flex align-items-center" v-show="imageDefault">
                        <span class="image-primary display-flex align-items-center">
                            <svg class="image-icon-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                                ...
                                <path fill="#FFF" d="M216.7 350.9h-.1c-5.1 0-9.9-2.1-13.4-5.7l-74.2-76c-7.4-7.5-7.2-19.5.4-26.8 7.5-7.4 19.5-7.2 26.8.4L217 305l139.7-138.5c7.5-7.4 19.5-7.4 26.8.1s7.4 19.5-.1 26.8l-153.2 152c-3.7 3.5-8.5 5.5-13.5 5.5z"></path>
                            </svg>
                            {{primaryText}}
                        </span>
                ...
                <div class="display-flex">
                    <label class="image-edit display-flex cursor-pointer" :for="imageEditId">
        ...
        <div class="image-list-container display-flex flex-wrap" v-if="images.length && multiple">
            <div class="image-list-item position-relative cursor-pointer" :class="image.highlight && 'image-highlight'" v-for="(image, index) in images" :key="index" @click="changeHighlight(index)">
                <div class="centered">
                    <img class="show-img img-responsive" :src="getImageThumb(image, 'icon')">
                </div>
            </div>
            <div class="image-list-item position-relative cursor-pointer display-flex justify-content-center align-items-center" v-if="!disabled && images.length < parseInt(maxImage)">
                ...
                <div class="input-add-image position-absolute full-width full-height">
                    <label :for="imageUploadId" class="display-block full-width full-height cursor-pointer">
                    </label>
                </div>
            </div>
        </div>
        <div>
            <input :id="imageUploadId" class="image-upload" @change="uploadFieldChange" name="images" :multiple="multiple" :accept="accept" type="file">
            <input :id="imageEditId" class="image-edit" @change="editFieldChange" name="image" :accept="accept" type="file">
        </div>

        <vue-image-lightbox-carousel ref="lightbox" :show="showLightbox" @close="showLightbox = false" :images="images" @change="changeHighlight" :showCaption="false">
...
<script>
    ...
    import pdfThumb from '../assets/filetype/pdf-thumb.png';
    
    import csv from '../assets/filetype/csv.png';
    import csvThumb from '../assets/filetype/csv-thumb.png';
    
    import xls from '../assets/filetype/xls.png';
    import xlsThumb from '../assets/filetype/xls-thumb.png';
    
    import doc from '../assets/filetype/doc.png';
    import docThumb from '../assets/filetype/doc-thumb.png';
    
    import other from '../assets/filetype/other.png';
    import otherThumb from '../assets/filetype/other-thumb.png';
    ...
    export default {
        ...
        computed: {
            imagePreview() {
                ...
                if (index > -1) {
                    ...
                    name = (this.images.length) ? this.images[0].name : ''
                    path = (this.images.length) ? this.images[0].path : ''
                }
                ...
                switch(ext) {
                    case 'pdf':
                        path = this.filetype.pdf.icon
                        break;
                    case 'xls':
                        path = this.filetype.xls.icon
                        break;
                    case 'csv':
                        path = this.filetype.csv.icon
                        break;
                    case 'doc':
                        path = this.filetype.doc.icon
                        break;
                    case 'docx':
                        path = this.filetype.docx.icon
                        break;
                    default:
                        if(ext != 'jpg' && ext != 'jpeg' && ext != 'png' && ext != 'png' && ext != 'gif') {
                            path = this.filetype.other.icon
                        }
                }
        ...
        methods: {
            onDrop(e) {
                ...
                if (document.getElementById(this.imageUploadId)) {
            ...
            editFieldChange(e) {
                ...
                if (document.getElementById(this.imageEditId)) {
                    document.getElementById(this.imageEditId).value = ''
                }
            },
            ...
            getImageThumb: function(image, type = 'thumb') {
                let path = image.name,
                    ext = path.split('.').pop()
                
                switch(ext) {
                    case 'pdf':
                        path = (type == 'icon') ? this.filetype.pdf.icon : this.filetype.pdf.thumb
                        break;
                    case 'xls':
                        path = (type == 'icon') ? this.filetype.xls.icon : this.filetype.xls.thumb
                        break;
                    case 'csv':
                        path = (type == 'icon') ? this.filetype.csv.icon : this.filetype.csv.thumb
                        break;
                    case 'doc':
                        path = (type == 'icon') ? this.filetype.doc.icon : this.filetype.doc.thumb
                        break;
                    case 'docx':
                        path = (type == 'icon') ? this.filetype.docx.icon : this.filetype.docx.thumb
                        break;
                    default:
                        if(ext != 'jpg' && ext != 'jpeg' && ext != 'png' && ext != 'png' && ext != 'gif') {
                            path = (type == 'icon') ? this.filetype.other.icon : this.filetype.other.thumb
                        } else {
                            path = image.path
                        }
                }
